var array= [
  {
    id: 1,
    category: "CHORBA",
    elems: [
      {

        name: "CHICKEN CORN SOUP",
        nameAr: "شوربة ذرة الدجاج",
        price: "37",
        description: '',
        descriptionAr:"",
      },
      {
        
        name: "MUSHROOM SOUP",
        nameAr: "حساء الفطر ",
        
        descriptionAr:"",
        price: "34",
        description: '',
      },
      {
        
        name: "FRENCH ONION SOUP",
        nameAr: "شوربة البصل",
        price: "30",
        description: '',
        descriptionAr:"",
      },
    ],
  },
  {
    id: 2,
   
    category: "GREENS",
    elems: [
      {
        
        name: "CEASAR SALAD",
        nameAr:"سلطة سيزر  ",
        price: "42",
        description: "Ice burg lattuce, Cherry Tomato,Chicken, Served with Ceaser Dressing",
        descriptionAr: "خس برجر ، طماطم كرزية ، دجاج ، تقدم مع صوص سيزر",

        
      },
      {
        
        name: "QUINUA SALAD",
        nameAr:"سلطة الكنوا",
        descriptionAr:"كينوا ، خضروات مشكلة ، تقدم مع صلصة الليمون والعسل ",
        price: "49",
        description: "QUINUA, Mixed vegetable, Served with Honey Lemon Dressing."
      },
      {
        
        name: "FRIED PRAWN SALAD",
        nameAr:"سلطة روبيان مقلي ",
        descriptionAr:"روبيان ، أفوكادو ، مشكل أوراق صغيرة ، رمان و مكسرات ، تقدم مع صلصة الرانش الخفيفة",
        price: "52",
        description: "Shrimp, avocado, Mix Baby Leaves, Pomegranates and Nuts, Served with Light Ranch Dressing.",
      },   {
        
        name: "HUMMUS",
        nameAr:"حمص ",
        descriptionAr:"",
        price: "32",
        description: "",
      },   {
        
        name: "MUTTABAL",
        nameAr:"متبل",
        descriptionAr:"",
        price: "30",
        description: "",
      },   {
        
        name: "FATTOUCH",
        nameAr:"فتوش ",
        descriptionAr:"",
        price: "28",
        description: "",
      },   {
        
        name: "TABOULAH",
        nameAr:"تبولة",
        descriptionAr:"",
        price: "26",
        description: "",
      },
   
    ],
  },
  {
    id: 3,
   
    category: "APPETIZERS",
    elems: [
    
      {
        
        name: "DYNAMITE SHRIMP",
        nameAr:"ديناميت روبيان",
        descriptionAr:"روبيان مقلي متبل بالمايونيز يقدم مع عسل و صوص سريراتشا",
        price: "58",
        description: "Fried Shrimp, Marinated with Mayonnaise, Served with Honey and Sriracha sauce.",
      },
      {
        
        name: "SPICY POTATOES",
        nameAr:"البطاطس الحارة",
        descriptionAr:"مكعبات شرائح البطاطس المخبوزة والمطبوخة بصلصة الكريمة",
        price: "38",
        description: "Cube Slice potatoes Baked and Cooked With Creamy Sauce.",
      },
      {
        
        name: "AFREEN KIBAH 6PCS",
        nameAr:"أفرين كبة 6 قطع",
        descriptionAr:"",
        price: "48",
        description: "",
      },
      {
        
        name: "RISOTTO BALLS",
        nameAr:"كرات الريزوتو",
        descriptionAr:"كرة أرز مقلية لذيذة ، مخبوزة مع جبنة حلوة في الوسط",
        price: "52",
        description: "Delicious Fried Rice Ball, Breaded with a Sweet Cheese in Center",
      },
     
      
      {
        
        name: "MOZARELLA STICKS",
        nameAr:"أصابع الموزاريلا",
        descriptionAr:"",
        price: "47",
        description: "",
      },{
        
        name: "STUFFED MUSHROOMS",
        nameAr:"فطر محشي",
        descriptionAr:"فطر صالح للأكل يقدم ساخنا",
        price: "55",
        description: "Edible Mushroom Backed & Served Hot.",
      },
      {
        
        name: "TENDER CHICKEN",
        nameAr:"دجاج طري",
        descriptionAr:"",
        price: "45",
        description: "",
      },
      {
        
        name: "SHRIMPS SAUCE & CHERRY",
        nameAr:"صلصة الروبيان والكرز",
        descriptionAr:"روبيان طازج ملفوف بالكنافة العربية ، يقدم مع صلصة الكرز الحلوة.",
        price: "53",
        description: "Fresh Tasty Shrimp Wrapped in Arabic Kunafa, Served with Sweet Cherry Sauce.",
      },
      {
        
        name: "AFREEN SPECIAL ROLLS",
        nameAr:"لفات خاصة أفرين",
        descriptionAr:"",
        price: "42",
        description: "",
      }
    ],
  },
  {
    id: 4,
    
    category: "FINGER BITES",
    elems: [
      { 
        name: "SAMBOSA ",
        nameAr:"سامبوسا",
        descriptionAr:"معجنات لذيذة محشوة بالدجاج المتبل بالبهارات الهندية",
        price: "32",
        description: "Savory pastries stuffed with Chicken  seasoned with indian spices.",
      },
      {
        
        name: "CLUB SANDWICH",
        nameAr:"كلوب ساندويتش",
        descriptionAr:"خبز ، بيض ، دجاج ، خس ومايونيز. اصنع قطعتي لير ثم شوي وتقدم مع البطاطا المقلية.",
        price: "55",
        description: "Bread, Egg, Chicken, Lettuce and Mayonnaise. Make the two lear then Grill and Served with French Fries.",
      },
      {
        
        name: "CHICKEN TIKKA ROLL",
        nameAr:"رول تكا دجاج",
        descriptionAr:"هي عبارة عن رول لذيذ حيث يتم لف قطع الدجاج المشوي في خبز روتي ناعم ومحشو بالخضار.",
        price: "35",
        description: "It is Delicious Roll Where pieces of Grilled Chicken is wrapped in a soft roti and Stuffed with Vegetable.",
      },
      {
        
        name: "CHICKEN LOLLIPOP",
        nameAr:"مصاصة الدجاج",
        descriptionAr:"أجنحة دجاج متبلة بالبهارات الهندية وصلصة الثوم الحارة ثم مقلية. تقدم مع الصلصة الحارة.",
        price: "45",
        description: "Chicken Wings marinated with Indian Spices and Hot Garlic Sauce then Deep Fried. Served with Hot sauce.",
      },  {
        
        name: "CREAM CHICKEN ROLL",
        nameAr:"رول دجاج بالكريمة",
        descriptionAr:"",
        price: "35",
        description: "",
      },
      {
        
        name: "CASADIA",
        nameAr:"كاساديا",
        descriptionAr:"",
        price: "49",
        description: "",
      },
      {
        
        name: "MINI BURGERS CHICKEN AND BEEF",
        nameAr:"ميني برجر دجاج ولحم بقري",
        descriptionAr:"",
        price: "55",
        description: "",
      }
    ],
  },
  {
    id: 5,
   
    category: "AROUND THE WORLD",
    elems: [
      {      
        name: "GRILLED SALMON",
        nameAr:"سلمون مشوي",
        descriptionAr:"سلمون نرويجي مشوي يقدم مع بطاطا مهروسة وخضار مشوي",
        price: "105",
        description: "Norwegian Salmon Grilled, Served with mash potato and grilled Vegetable",
      },
      {
        
        name: "TENDERLOIN BITES",
        nameAr:"مقبلات تندرلوين  ",
        descriptionAr:"",
        price: "99",
        description: "",
      }, 
      {
        
        name: "CHICKEN PISTACHIO",
        nameAr:"دجاج بالفستق",
        descriptionAr:"برست الدجاج المشوي المقرمش مع كرم الفستق ومدعوم بإتقان العصير. تقدم مع بطاطس مهروسة ، خضار مطبوخ على البخار وقطرة من صلصة الفستق",
        price: "95",
        description: "Grilled Chicken Brest Crusted with pistachio Crum and backed to Juice Perfection. Served with Mash Potatoes, Steamed vegetable and drip of Pistachio Sauce.",
      }, 
      {
        
        name: "SALT PEPPER PRAWNS",
        nameAr:"روبيان مملح و حار",
        descriptionAr:"",
        price: "87",
        description: "",
      }
   
    ],
  },{
    id: 6,
   
    category: "MAMMA MIA",
    elems: [
   
  
    
      {
        
        name: "CHICKEN FETTUCCINI",
        nameAr:"فيتوتشيني الدجاج",
        descriptionAr:"",
        price: "62",
        description: ""
      }, {
        
        name: "PENNE PASTA",
        nameAr:"بيني باستا",
        descriptionAr:"",
        price: "68",
        description: ""
      }, {
        
        name: "PESTO PASTA",
        nameAr:"باستا البيستو",
        descriptionAr:"",
        price: "79",
        description: ""
      }
    ],
  },
  {
    id: 7,
   
    category: "DESI FLAVOURS",
    elems: [
      {
        
        name: "PLAIN BIRYANI",
        nameAr:"برياني سادة",
        descriptionAr:"",
        price: "45",
        description: ""
      },   {
        
        name: "CHICKEN BIRYANI",
        nameAr:"برياني دجاج",
        descriptionAr:"أرز بسمتي عطري مطبوخ بقطع طرية من الدجاج والأعشاب والبهارات ، يقدم مع المرق وسلطة الزبادي.",
        price: "62",
        description: "Aromatic Basmati Rice Cooked with tender pieces of Chicken, Herb and Spices, Served with gravy and Yoghurt Salad."
      },   {
        
        name: "MUTTON BIRYANI",
        nameAr:"برياني غنم",
        descriptionAr:"",
        price: "75",
        description: ""
      },
      {
        
        name: "BUTTER CHICKEN",
        nameAr:"دجاج بالزبدة",
        descriptionAr:"قطع دجاج منزوعة العظم مطبوخة في مرق الزبدة والكاجو والطماطم مع الكريمة.",
        price: "55",
        description: "Boneless pieces of Chicken Cooked in Butter, Cashew and Tomato Gravy with Cream."
      },{
        
        name: "MURGH TIKKA MASALA",
        nameAr:"مرغ تيكا ماسالا",
        descriptionAr:"دجاج باربيكيو طري مطبوخ بصلصة الكريمة مع الطماطم الطازجة والتوابل الشرقية.",
        price: "60",
        description: "Tender BBQ Chicken Cooked in Creamy Sauce with fresh tomatoes and oriental Spices."
      }
    ],
  },{
    id: 8,
   
    category: "TANDOOR SPECIAL grilled with love",
    elems: [
      {
        
        name: "CHICKEN TIKKA",
        nameAr:"تيكا دجاج",
        descriptionAr:"قطع دجاج متبلة بالليمون والتوابل واللبن الرائب ومغطاة بالتندور. (تقدم مع صوص النعناع)",
        price: "65",
        description: "Chicken Pieces Marinated in Lemon, Spices & Hung Curd, Coocked in Tandoor. (Served with Mint Sauce)"
      },
      {
        
        name: "MUTTON SEEKH",
        nameAr:"سيخ غنم",
        descriptionAr:" لحم الضأن المفروم المتبل عبارة عن بهارات هندية مشوية على التندور (تقدم مع حلقات البصل ، شرائح الليمون ، صلصة النعناع)",
        price: "75",
        description: "Maid with Minced Lamb Marinatedis verity of Indian Spices Grilled on a Tandoor (Served with onion rings, Lemon Wedges, Mint Sauce)"
      },{
        
        name: "KING BUTTER PRAWN",
        nameAr:"جمبرى مع الزبدة",
        descriptionAr:"روبيان كينج  متبل بالثوم ، عصير ليمون طازج ، بذور كاروم ، بهارات ، مطبوخ بالتندور. (تقدم مع صوص النعناع)",
        price: "85",
        description: "Jumbo King Prawns Marinated in Garlic Past, Fresh Lemon juice, Carom Seeds, Spices, Hung Curd, Cooked in tandoor. (Served with Mint Sauce)"
      },{
        
        name: "MIX GRILL",
        nameAr:"مشاوي مشكل",
        descriptionAr:"",
        price: "125",
        description: ""
      }
    ]
  },{
    id: 9,
   
    category: "NAAN DIRECTLY FROM TANDOOR",
    elems: [
      {
        
        name: "PLAIN NAAN",
        nameAr:"خبز نان",
        descriptionAr:"عبارة عن خبز مسطح طري مخمّر ، مصنوع من الدقيق الأبيض ، يُطهى تقليديًا في التندور",
        price: "6",
        description: "Nan is a soft, leavened flat bread, made with white flour, That's Traditionally Cooked in a tandoor"
      },
      {
        
        name: "BUTTER NAAN",
        nameAr:"نان بالزبدة",
        descriptionAr:"",
        price: "8",
        description: ""
      },{
        
        name: "GARLIC NAAN",
        nameAr:"نان مع الثوم",
        descriptionAr:"نان المطبوخ مع الثوم المفروم الطازج وقليل من الكزبرة.",
        price: "8",
        description: "Our Signature Naan Cooked with Freshly chopped Garlic & a Hint of Coriander."
      },{
        
        name: "ASSORTED BASKET",
        nameAr:"سلة متنوعة",
        descriptionAr:"",
        price: "26",
        description: ""
      }
    ]
  },{
    id: 10,
   
    category: "WINTER HEATER",
    elems: [
      {
        
        name: "FLAT WHITE",
        nameAr:"فلات وايت",
        descriptionAr:"",
        price: "28",
        description: ""
      },
      {
        
        name: "TURKISH COFFE",
        nameAr:"قهوة تركي",
        descriptionAr:"",
        price: "30",
        description: ""
      },{
        
        name: "CAFE LATTE",
        nameAr:"قهوة بالحليب",
        descriptionAr:"",
        price: "30",
        description: ""
      },{
        
        name: "SPANISH LATE",
        nameAr:"لاتاي اسباني",
        descriptionAr:"",
        price: "35",
        description: ""
      },{
      
        name: "MORROCAN TEA",
        nameAr:"شاي مغربي",
        descriptionAr:"",
        price: "42",
        description: ""
      },{
        
        name: "RED TEA KETTLE",
        nameAr:"غلاية الشاي الأحمر",
        descriptionAr:"",
        price: "45",
        description: ""
      },{
        
        name: "V60",
        nameAr:"في60",
        descriptionAr:"",
        price: "35",
        description: ""
      }
    ]
  },{
    id: 11,
   
    category: "AFREEN SIGNATURE MOCKTAILS",
    elems: [
      {
        name: "PASSION FRUIT ICE TEA",
        nameAr:"باشن فروت آيس تي",
        descriptionAr:"شاي مثلج مصنوع منزليًا في أفضل أنواع الشاي الممزوج بالباشون فروت وعصير الليمون وشراب السكر وشراب الفانيليا",
        price: "45",
        description: "homemade ice tea at it’s best infused tea-passionfruit puree-lemon juice-sugar syrup-vanilla syrup"
      },
      {
        
        name: "SANGRIA",
        nameAr:"سانغريا",
        descriptionAr:"عصير تفاح ، عصير عنب ، هريس فاكهة العاطفة ، بياض بيض ، عصير ليمون ، تفاح أخضر ، تفاح أحمر",
        price: "42",
        description: "Apple juice, grape juice, passion fruit puree, egg white, lemon juice, green apple, red apple."
      },{
        
        name: "THYME IS UP!",
        nameAr:"تايم ايز آب",
        descriptionAr:"مزيج من شراب الزعتر الحلو والمر - المر - شراب الليتشي - الليمون - الزنجبيل",
        price: "48",
        description: "a combination of sweet and bitter- thyme syrup-bitters-lychee syrup-lemon-ginger ale "
      },{
        
        name: "LEMON ICE TEA",
        nameAr:"آيس تي مع الليمون",
        descriptionAr:"مصنوع منزليًا في أفضل أنواع الشاي وعصير الليمون وشراب السكر وشراب الفانيليا",
        price: "48",
        description: "homemade ice tea at it’s best-infused tea-lemon juice-sugar syrup-vanilla syrup"
      },{
        
        name: "GUMBALL",
        nameAr:"غامبول",
        descriptionAr:"عصير الحلوى - شراب الفقاعات - شراب الفراولة - عصير الليمون - النعناع - ماء الليمون",
        price: "45",
        description: "let me try some candy juice –bubble gum syrup-strawberry syrup-lemon juice-mint-lemonade water"
      },{
        
        name: "BERRY BLUST",
        nameAr:"باري بلاست",
        descriptionAr:"مشروبات اولر مع فواكه توت مشكلة - عصير توت ، توت اسود - عصير تفاح - عصير زنجبيل - شراب فراولة - خلطة توت - اوراق نعناع",
        price: "48",
        description: "cooler drinks with mix berries fruits- raspberry juice, blueberry-blackberry-apple juice-ginger juice-strawberry syrup-mix berries-mint leaves"
      },{
        
        name: "YUZU TERRIFIC!",
        nameAr:"يوزو تريفك",
        descriptionAr:"مزيج حامض - برتقال محروق - برتقال - عصير طازج - أوراق ليمون - مزيج حامض يوزو        ",
        price: "48",
        description: "Yuzu refreshing sour mix –burnt orange-orange juice fresh-lime leaf-yuzu mix-sour mix"
      },{
        
        name: "SAUDI CHAMPAGNE",
        nameAr:"شامبانيا سعودية",
        descriptionAr:"صُنع في أفرين بار",
        price: "70",
        description: "tailored made at Afreen bar"
      },
      {
        
        name: "PEACH ME UP!",
        nameAr:"بيتش مي آب",
        descriptionAr:"اشعر باندفاع الخوخ في فمك - غير الجن نباتي مدمن على الكحول - هريس الخوخ - عصير التوت البري = مزيج حامض ",
        price: "48",
        description: "feel the rush of peach in your mouth –non alcoholic gin botanical-peach puree-cranberry juice=sour mix"
      },  {
        
        name: "I GOT HOT SAUCE!",
        nameAr:"آي غوت هوت صوص",
        descriptionAr:"حلوة ولمسة من البهارات - توت العليق - عصير أناناس - عصير ليمون - شراب سكر - توباسكو",
        price: "45",
        description: "sweet and touch of spice – raspberry fruit-pineapple juice-lemon juice-sugar syrup-tobasco"
      }
    ]
  },{
    id: 12,
   
    category: "HIGH TEA",
    elems: [
      {
        name: "HIGH TEA COMBO",
        nameAr:"هاي تي كومبو",
        descriptionAr:"كرواسان ، ماكرون ، فواكه ، وعاء شاي 2-4",
        price: "45",
        description: "CROISSANT, MACARONS, FRUITS, TEA POT 2-4"
      },]},{
        id: 13,
       
        category: "SHISHA",
        elems: [
          {
            name: "SHISHA YOUR CHOICE",
            nameAr:"شيشة",
            descriptionAr:"",
            price: "99",
            description: ""
          }]}
];
export default array
